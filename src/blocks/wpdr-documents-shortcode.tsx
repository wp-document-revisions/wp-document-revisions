...
const DocumentsShortcodeEdit = ({ attributes, setAttributes }: DocumentsShortcodeBlockProps) => {
	...
	const inspectorControls = (
		<InspectorControls>
			<PanelBody title={__('Documents Settings', 'wp-document-revisions')} initialOpen={true}>
				<TextControl
					label={__('Header', 'wp-document-revisions')}
					value={header}
					onChange={(value: string) => setAttributes({ header: value })}
					help={__(
						'Optional header text for the documents list',
			...
			<PanelBody title={__('Taxonomy Filters', 'wp-document-revisions')} initialOpen={false}>
				<RadioControl
			...
			<PanelBody title={__('Advanced', 'wp-document-revisions')} initialOpen={false}>
				<TextareaControl
					label={__('Additional Shortcode Parameters', 'wp-document-revisions')}
					value={freeform}
					onChange={(value: string) => setAttributes({ freeform: value })}
					help={__(
						'Additional shortcode parameters in key="value" format',
						'wp-document-revisions'
					)}
					rows={3}
				/>
