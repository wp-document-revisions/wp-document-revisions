...
class Test_WP_Document_Revisions_Widget extends Test_Common_WPDR {
	...
	// phpcs:enable
		...
		$terms = wp_set_post_terms( self::$author_public_post, array( self::$ws_term_id ), 'workflow state' );
		...
		// add term and attachment.
		$terms = wp_set_post_terms( self::$editor_private_post, array( self::$ws_term_id ), 'workflow state' );
		self::assertTrue( is_array( $terms ), 'Cannot assign workflow state to document' );
