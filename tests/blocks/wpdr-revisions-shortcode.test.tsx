...
describe('Revisions Shortcode Block', () => {
  ...
  const config = (blocks.registerBlockType as any).mock.calls[0][1];
    expect(config.attributes).toEqual(expect.objectContaining({
  ...
  const { blocks } = loadBlock();
  const config = (blocks.registerBlockType as any).mock.calls[0][1];
    expect(config.supports).toEqual(expect.objectContaining({
      align: true,
  ...
  const { blocks } = loadBlock();
  const config = (blocks.registerBlockType as any).mock.calls[0][1];
    expect(config.keywords).toEqual(expect.arrayContaining(['revisions','history','versions','document']));
  });
