...
jest.mock('@wordpress/components', () => ({
  ...
  RangeControl: jest.fn(({ label, value }) => (
    <div data-testid="range-control">{label}: {value}</div>
  )),
  TextControl: jest.fn(({ label, value }) => (
    <div data-testid="text-control">{label}: {value}</div>
  )),
  ToggleControl: jest.fn(({ label, checked }) => (
    <div data-testid="toggle-control">{label}: {checked ? 'Yes' : 'No'}</div>
  ))
}));
...
describe('Documents Widget Block', () => {
  ...
  describe('Block Registration', () => {
    it('should register the documents widget block', () => {
      ...
      expect(mock.calls[0][1]).toEqual(expect.objectContaining({
    ...
    it('should have correct widget attributes', () => {
      ...
      expect(blockConfig.attributes).toEqual(expect.objectContaining({
    ...
    it('should support block customization features', () => {
      ...
      expect(blockConfig.supports).toEqual(expect.objectContaining({
        align: true,
        color: expect.objectContaining({ background: true, text: true, link: true, gradients: true }),
    ...
    it('should include appropriate keywords', () => {
      const mock = loadAndGet();
      const blockConfig = mock.calls[0][1];
      expect(blockConfig.keywords).toEqual(expect.arrayContaining(['documents','latest','recent','widget']));
    });
  });
