...
class Test_WP_Document_Revisions_Edge_Cases extends Test_Common_WPDR {
	...
	public function test_update_post_slug_field_unauthorized() {
		...
		self::assertTrue( false !== strpos( $exception->getMessage(), 'Not authorized' ), 'Exception message should indicate authorization failure' );
	...
	public function test_update_post_slug_field_authorized() {
		...
		if ( null !== $exception ) {
			self::assertTrue(
				false === strpos( $exception->getMessage(), 'Not authorized' ),
